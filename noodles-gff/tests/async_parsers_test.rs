...
use noodles_gff::feature::{record_buf::attributes::field::Value, record::{Strand, Phase}, RecordBuf};
...
async fn test_fast_parser_with_real_records() -> io::Result<()> {
    ...
    if let Some(Value::String(gene_type)) = attributes.get(b"gene_type") {
        assert_eq!(gene_type.as_ref() as &[u8], b"transcribed_unprocessed_pseudogene");
    } else {
...
async fn test_parser_consistency() -> io::Result<()> {
    ...
    for (i, (fast_record, simd_record)) in fast_records.iter().zip(simd_records.iter()).enumerate() {
        // Basic fields
        assert_eq!(fast_record.reference_sequence_name(), simd_record.reference_sequence_name(), "Record {} reference sequence name mismatch", i);
        assert_eq!(fast_record.source(), simd_record.source(), "Record {} source mismatch", i);
        assert_eq!(fast_record.ty(), simd_record.ty(), "Record {} type mismatch", i);
        assert_eq!(fast_record.start(), simd_record.start(), "Record {} start mismatch", i);
        assert_eq!(fast_record.end(), simd_record.end(), "Record {} end mismatch", i);
        assert_eq!(fast_record.score(), simd_record.score(), "Record {} score mismatch", i);
        assert_eq!(fast_record.strand(), simd_record.strand(), "Record {} strand mismatch", i);
        assert_eq!(fast_record.phase(), simd_record.phase(), "Record {} phase mismatch", i);
        
        // Attributes
        let fast_attrs = fast_record.attributes();
        let simd_attrs = simd_record.attributes();
        
        assert_eq!(fast_attrs.len(), simd_attrs.len(), "Record {} attributes length mismatch", i);
        ...
        for (key, fast_value) in fast_map {
            let simd_value = simd_attrs.get(key).expect(&format!("Record {} missing attribute {}", i, std::str::from_utf8(key).unwrap()));
            
            match (fast_value, simd_value) {
                (Value::String(fast_str), Value::String(simd_str)) => {
                    assert_eq!(fast_str.as_ref() as &[u8], simd_str.as_ref() as &[u8], "Record {} attribute {} string value mismatch", i, std::str::from_utf8(key).unwrap());
                }
                (Value::Array(fast_arr), Value::Array(simd_arr)) => {
                    assert_eq!(fast_arr.len(), simd_arr.len(), "Record {} attribute {} array length mismatch", i, std::str::from_utf8(key).unwrap());
                    for (j, (fast_item, simd_item)) in fast_arr.iter().zip(simd_arr.iter()).enumerate() {
                        assert_eq!(fast_item.as_ref() as &[u8], simd_item.as_ref() as &[u8], "Record {} attribute {} array item {} mismatch", i, std::str::from_utf8(key).unwrap(), j);
                    }
                }
                _ => panic!("Record {} attribute {} type mismatch between parsers", i, std::str::from_utf8(key).unwrap()),
            }
        }
    }
